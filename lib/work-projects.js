...
export async function updateProjectsOrder(projectOrders) {
  try {
    const response = await fetch('/api/work/projects-order', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      console.error('Error updating projects order:', response.statusText);
      return false;
    }
